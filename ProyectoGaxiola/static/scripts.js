function validateCodeStructure(code, language) {
    ...
    if (language === "python") {
        ...
        if (!functionPattern.test(code) && !classPattern.test(code)) {
            errors.push("Falta declaración de función o clase con ':' en Python.");
        }
        
        if (code.includes("def") && !code.includes(":")) {
            errors.push("Falta ':' en definición de función.");
        }
        ...
        const classPattern = /class\s+\w+/;
        const mainPattern = /static\s+void\s+Main\s*\(string\[\]\s+args\)/;

        if (!usingPattern.test(code)) {
            errors.push("Falta declaración de 'using' en C#.");
        }
        
        if (!namespacePattern.test(code)) {
            errors.push("Falta declaración de 'namespace' en C#.");
        }

        if (!classPattern.test(code)) {
            errors.push("Falta declaración de clase en C#.");
        }

        if (!mainPattern.test(code)) {
            errors.push("Falta declaración de 'Main' en C#.");
        }
        ...
        if (!functionPattern.test(code) && !variablePattern.test(code)) {
            errors.push("Falta declaración de función o variable en JavaScript.");
        }
...
// Modificar el código principal para incluir la función de verificación y manejar errores antes de la conversión
document.getElementById('convertButton').addEventListener('click', () => {
    const inputLanguage = document.getElementById('inputLanguage').value;
    const outputLanguage = document.getElementById('outputLanguage').value;
    const codeInput = document.getElementById('codeInput').value;

    // Validación de la estructura del código antes de la conversión
    const validationErrors = validateCodeStructure(codeInput, inputLanguage);
    ...
    // Llamar a los motores de traducción (sin cambios en esta parte)
